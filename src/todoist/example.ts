...
import { currentDate, parseFunctionCall, rephrase } from "./helper";
import { addTasks, closeTasks, listUncompleted, updateTasks } from "./todoist";
import {
  addTasksSchema,
  finishTasksSchema,
  getTasksSchema,
...
const act = async (query: string) => {
  ...
  const conversation = await model.invoke([
    new SystemMessage(`
            ...
            Current tasks: ###${tasks
              .map((task: any) => task.content + " (ID: " + task.id + ")")
              .join(", ")}###`),
